...
const CONTENT = [
    ...
    "- `bindery_get_text(path)` — read settings, arc files, or memory files",
    "- `bindery_search(query, language)` — find thematic parallels and related moments across the book",
    "- `bindery_get_notes(category, name)` — look up character profiles, world rules, or equipment details",
    "- `bindery_get_chapter(chapterNumber, language)` — read a specific chapter for reference",
    "",
    "## Phases",
    "",
    ...
    "",
    "- Always: `bindery_get_text(\".bindery/settings.json\")`, `bindery_get_text(\".bindery/memories/global.md\")`, and the relevant arc file from `Arc/`",
    "- If chapter-specific: `bindery_get_text(\".bindery/memories/chXX.md\")` if it exists",
    "- If character-focused: `bindery_get_notes(category: \"Characters\")`",
    "- Always: `bindery_search` for related moments or themes already in the book",
    "",
    "### Phase 3 — Discuss",
