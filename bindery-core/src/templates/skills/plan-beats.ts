...
const CONTENT = [
    ...
    "- `bindery_get_chapter(chapterNumber, language)` — read an existing chapter and any embedded beatmap",
    "- `bindery_get_text(path)` — read settings, `PREFERENCES.md`, `.bindery/README.md`, or memory files",
    "- `bindery_arc_list` / `bindery_arc_get` / `bindery_arc_update` — read or update structured arc files that hold beatmaps",
    "- `bindery_character_list` / `bindery_character_get` — load relevant structured character profiles",
    "- `bindery_get_notes(category, name)` — look up characters, world rules, or equipment",
    "- `bindery_search(query, language)` — find parallel moments or thematic echoes across the book",
    "",
    ...
    "2. Read `.bindery/settings.json` with `bindery_get_text` (genre, target audience, story structure).",
    ...
    "6. If the chapter is already written, read it with `bindery_get_chapter` for scene structure and prose context.",
    ...
    "8. If character-driven beats are involved: call `bindery_character_list`, then `bindery_character_get` for relevant profiles. Use `bindery_get_notes(category: \"Characters\")` only for older note layouts.",
    "9. Call `bindery_search` to check for thematic echoes or parallel beats elsewhere in the book.",
    "",
    "### Phase 5 — Discuss",
