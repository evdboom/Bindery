...
const CONTENT = [
    ...
    "- `bindery_get_chapter(chapterNumber, language)` — read a chapter in any language (source or existing translation)",
    "- `bindery_get_translation(targetLanguage)` — list glossary entries for a target language (e.g. `\"nl\"`)",
    "- `bindery_get_translation(targetLanguage, word)` — look up a specific term; forgiving: case-insensitive, handles plurals and inflected forms",
    "- `bindery_search(query, targetLanguage)` — verify how a term was rendered in other translated chapters",
    "- `bindery_add_translation(targetLanguage, from, to)` — save a new glossary term pair when the user confirms a translation choice",
    "",
    "## Steps",
    "",
    "### 1. Load the translation table",
    "Call `bindery_get_translation(targetLanguage)` to load all known glossary term mappings for the target language before translating anything.",
    "",
    "### 2. Load the chapter",
    "Use `bindery_get_chapter(chapterNumber, sourceLanguage)` to read the source chapter.",
    "For spot-check mode, also call `bindery_get_chapter(chapterNumber, targetLanguage)` to read the existing translation.",
    "",
    ...
    "When the user confirms a new or corrected term translation, call `bindery_add_translation` to persist it as a glossary entry. For spelling variant rules (dialect substitutions applied at export), use `bindery_add_dialect` instead.",
    "",
    "## Rules",
